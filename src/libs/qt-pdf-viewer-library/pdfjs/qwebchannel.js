...
** @version: 5.15.2
**
** @note: This file has been modified to support older browsers (ES<6), so lambdas and other methods
** have been replaced with older methods.
**
...
var QWebChannel = function(transport, initCallback)
{
    ...
    channel.exec({type: QWebChannelMessageTypes.init}, function(data) {
        ...
        for (var i=0; i<dataKeys.length; i++) {
            var objectName = dataKeys[i]

            new QObject(objectName, data[objectName], channel);
        }

        // now unwrap properties, which might reference other registered objects
        var channelKeys = Object.keys(channel.objects)
        for (var j=0; j<channelKeys.length; j++) {
            var channelObjectName = channelKeys[j]
            channel.objects[channelObjectName].unwrapProperties();
        }
...
function QObject(name, data, webChannel)
{
    ...
    this.unwrapQObject = function(response)
    {
        if (response instanceof Array) {
            ...
            return response.map(function(qobj){object.unwrapQObject(qobj)})
        }
    ...
    {
        var propertyKeys = Object.keys(object.__propertyCache__)
        for (var i=0; i<propertyKeys.length; i++) {
            var propertyIdx = propertyKeys[i]
    ...
    this.propertyUpdate = function(signals, propertyMap)
    {
        ...
        var propertyKeys = Object.keys(propertyMap)
        for (var i=0; i<propertyKeys.length; i++) {
            var propertyIndex = propertyKeys[i]
            var propertyValue = propertyMap[propertyIndex];
            object.__propertyCache__[propertyIndex] = this.unwrapQObject(propertyValue);
        }

        var signalsKeys = Object.keys(signals)
        for (var j=0; j<signalsKeys.length; j++) {
            var signalName = signalsKeys[j]
...
// In ES<6 Object.assign is not defined, so for compatibility with
// old browsers this method is redefined.
